...
mcp-server-ozon-seller --env /path/to/.env

# CLI с env-файлом
ozon-seller-cli --env /path/to/.env <command>
```

`--env` загружает переменные из файла (формат `KEY=VALUE`) до инициализации сервера. Без `--env` — стандартные переменные окружения.
...
- **Новый инструмент** — добавить строку в таблицу доменов или `docs/<domain>.md`.
- **Новая CLI-команда** — добавить в раздел «CLI-режим» → «Примеры команд».
...
- **В КАЖДОМ PR** обновлять версию в `pyproject.toml`, `src/mcp_server_ozon_seller/__init__.py` и `server.json` (patch для фиксов, minor для новых фич).
