...
Создаст `dist/mcp_server_ozon_seller-X.Y.Z.tar.gz` и `.whl`.
...
```bash
pip install mcp-server-ozon-seller==X.Y.Z
```
...
Валидация проверит:
- Пакет `mcp-server-ozon-seller` существует на PyPI
- В README на PyPI есть строка `mcp-name: io.github.dontsovcmc/ozon-seller`
...
2. `src/mcp_server_ozon_seller/__init__.py` — `__version__`
